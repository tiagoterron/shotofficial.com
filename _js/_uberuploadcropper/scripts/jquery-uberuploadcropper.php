	...
	$.fn.uberuploadcropper = function(options){
		...
		var allUploadsComplete = function(){
			...
			$.each(imgdata,function(i,obj){
				var btn = {};
				var ddd = strReplaceChr(obj.name);
                ddd = replaceAll(ddd, " ", "-");
				var ip = '<?php echo preg_replace("/\./", "", $_SERVER['REMOTE_ADDR']); ?>';
				var str = '<div style="text-align: center;"><img src="'+ options.folder +'/'+ip+"-"+ ddd +'?d='+ (new Date()).getTime() +'" class="ubercropimage" id="ubercropimage_'+ i +'" style="margin: 0 auto;" /></div>'+
					'<input type="hidden" name="imgcrop['+ i +'][name]" id="imgcrop_'+ i +'_name" value="'+ddd+'" class="imgcrop_name" />'+
