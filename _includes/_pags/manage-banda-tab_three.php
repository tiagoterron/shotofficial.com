...
$filter = "(a.nome LIKE '%".$txt_search."%' OR a.instrumento LIKE '%".$txt_search."%' OR a.message LIKE '%".$txt_search."%')";	
}else{
$filter = "1=1";	
}
...
//TABELA E COLUNAS
$tabela = "tb_membros a";
$cols = array('ID{20}'=>'id{a}', 'Foto{150}'=>'imagem{a}', 'Nome{150}'=>'nome{a}', 'Instrumento{125}'=>'instrumento{a}', 'Clicks na foto{125}'=>'clicks{a}', 'Status{75}'=>'status{a}', 'Última Atualização{150}'=>'dt_cad{a}');
...
$SQL = "SELECT $columns FROM $tabela WHERE $filter ORDER BY a.position ASC";

$SQL = mysqli_query($connect, $SQL) or die(mysqli_error($connect));
$quantreg = mysqli_num_rows($SQL);
$SQL_PAG = "SELECT *, DATE_FORMAT(dt_cad, '%d/%m/%Y as %Hh%im') as dt_cad2, $columns FROM $tabela WHERE $filter ORDER BY a.position ASC LIMIT $inicial,$numreg";
...
<label>titulo: </label>
    ...
    <div class="top-titulo">Discografia Completa<br /><img src="<?php echo PATH ?>_images/manage/hr.jpg" /></div>
...
<ul class="label">
	...
	?>
    <li class="w225"><span class="l"><a href="<?php echo PATH ?>manage/banda-tab_three_adicionar/edit/<?php echo $SQL_Q["id"]?>">EDITAR</a> | <a href="<?php echo PATH ?>manage/banda-view/<?php echo $SQL_Q["id"]?>">DETALHES</a> | <?php if($SQL_Q["status"]=="on"){?><a href="<?php echo PATH ?>manage/exec/banda-tab_two-set-stats/<?php echo $SQL_Q["id"]?>">OFFLINE</a><?php }else{ ?><a href="<?php echo PATH ?>manage/exec/banda-tab_two-set-stats/<?php echo $SQL_Q["id"]?>">ONLINE</a><?php } ?></span></li>
    <div style="clear:both;"></div>
    </ul>
    <?php
	}
