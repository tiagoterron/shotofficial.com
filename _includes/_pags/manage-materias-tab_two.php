...
$SQL = "SELECT $columns FROM $tabela WHERE type = 'post' AND $filter ORDER BY a.dt_cad DESC";
$SQL = mysqli_query($connect, $SQL) or die(mysqli_error($connect));
$quantreg = mysqli_num_rows($SQL);
$SQL_PAG = "SELECT DATE_FORMAT(a.`dt_cad`, '%d/%m/%Y %Hh%im') as `dt_cad2`, $columns FROM $tabela WHERE type = 'post' AND $filter ORDER BY a.dt_cad DESC LIMIT $inicial,$numreg";
...
<div class="manage-content-menu">
	<ul>
        <li class="current"><a href="<?php echo PATH.$pag_atual ?>">NEWS</a><div class="right">&nbsp;</div></li>
    </ul>
</div>
...
<ul class="label">
	...
	?>
    <li class="w200"><span class="l"><a href="<?php echo PATH ?>manage/materias-tab_adicionar/edit/<?php echo $SQL_Q["id"]?>">EDIT</a> | <?php if($SQL_Q["status"]=="on"){?><a href="<?php echo PATH ?>manage/exec/materia-offline-two/<?php echo $SQL_Q["id"]?>">OFFLINE</a><?php }else{ ?><a href="<?php echo PATH ?>manage/exec/materia-online-two/<?php echo $SQL_Q["id"]?>">ONLINE</a><?php } ?> | <a href="<?php echo PATH ?>manage/exec/delete-materia-tab-two/<?php echo $SQL_Q["id"]?>">DELETE</a></span></li>
    <div style="clear:both;"></div>
    </ul>
    <?php
	}
