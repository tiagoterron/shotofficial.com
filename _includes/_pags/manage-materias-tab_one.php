...
$cols = array('ID{20}'=>'id{a}', 'Title{200}'=>'t_titulo{a}', 'Member{150}'=>'t_name{b}', 'Clicks{50}'=>'clicks{a}', 'Status{75}'=>'status{a}', 'Date{150}'=>'dt_cad{a}');
...
$SQL = "SELECT $columns FROM $tabela WHERE $filter AND a.dt_cad >= '$dt_last_week' AND type = 'post' ORDER BY a.dt_cad DESC";
$SQL = mysqli_query($connect, $SQL) or die(mysqli_error($connect));
$quantreg = mysqli_num_rows($SQL);
$SQL_PAG = "SELECT DATE_FORMAT(a.`dt_cad`, '%d/%m/%Y %Hh%im') as `dt_cad2`, $columns FROM $tabela WHERE $filter AND a.dt_cad >= '$dt_last_week' AND type = 'post' ORDER BY a.dt_cad DESC LIMIT $inicial,$numreg";
...
<div class="manage-content-menu">
	<ul>
    	<li class="current"><a href="<?php echo PATH.$pag_atual ?>">Recents</a><div class="right">&nbsp;</div></li>
        <li><a href="<?php echo PATH ?>manage/materias-tab_two/">All</a><div class="right">&nbsp;</div></li>
    </ul>
</div>
...
<label>titulo: </label>
    ...
    <div class="top-titulo">Recent posts<br /><img src="<?php echo PATH ?>_images/manage/hr.jpg" /></div>
...
<ul class="label">
	...
	?>
    <li class="w225"><span class="l"><a href="<?php echo PATH ?>manage/materias-tab_adicionar/edit/<?php echo $SQL_Q["id"]?>">EDIT</a> | <?php if($SQL_Q["status"]=="on"){?><a href="<?php echo PATH ?>exec/anuncio-set-stats/<?php echo $SQL_Q["id_a"]?>">OFFLINE</a><?php }else{ ?><a href="<?php echo PATH ?>exec/anuncio-set-stats/<?php echo $SQL_Q["id_a"]?>">ONLINE</a><?php } ?> | <a href="<?php echo PATH ?>manage/exec/delete-materias-tab_one/<?php echo $SQL_Q["id"]?>">DELETE</a></span></li>
    <div style="clear:both;"></div>
    </ul>
    <?php
	}
	...
	?>
<div class="both"></div>
<div class="manage_btn_adicionar"><a href="<?php echo PATH ?>manage/materias-tab_adicionar/"><img src="<?php echo PATH ?>_images/manage/btn-adicionar.jpg" class="manage_btn_adicionar" /></a></div>
<div class="paginacao">
